...
const employeeHandler = async (request, h) => {
  try {
    ...
    const getDetails = await prisma.$queryRaw`SELECT e.firstname, e.lastname, e.email, e.mobile, e.roledescription, e.departmentid, u.id as userId, u.username, u.role FROM public.employee e INNER JOIN public.userlogin u ON e.userid = u.id WHERE e.id = ${employeeId} AND visible = true;`;
